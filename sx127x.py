...
REG_FRF_LSB = 0x08
REG_PA_CONFIG = 0x09
REG_OCP = 0x0b # overcurrent protection，new
REG_LNA = 0x0c
...
class SX127x:
    ...
    def __init__(self,
        ...
        if "dio_0" in self._pins:
            fm.register(self._pins["dio_0"],fm.fpioa.GPIOHS10)
            self._pin_rx_done = GPIO(GPIO.GPIOHS10,GPIO.IN)
        if "ss" in self._pins:
            fm.register(self._pins["ss"],fm.fpioa.GPIOHS5)
            self._pin_ss = GPIO(GPIO.GPIOHS5,GPIO.OUT)
            #self._pin_ss = self._spi.CS0
        if "led" in self._pins:
            fm.register(self._pins["led"],fm.fpioa.GPIOHS11)
            self._led_status = GPIO(GPIO.GPIOHS11,GPIO.OUT)
    ...
    def transfer(self, address, value = 0x00):
        ...
        self._spi.write(bytes([address]),cs=self._spi.CS0)
        self._spi.write_readinto(bytes([value]), response,cs=self._spi.CS0)
